...
The Cypress suite assumes a running local backend, Keycloak realm, and two fixed users:

- owner user: `CYPRESS_OWNER_USERNAME` / `CYPRESS_OWNER_PASSWORD`
- invited user email for invite-only checks: `CYPRESS_INVITED_USER_EMAIL`
- guest user login: `CYPRESS_GUEST_EMAIL` / `CYPRESS_OWNER_PASSWORD`
- guest display name after login: `CYPRESS_GUEST_USERNAME`

The local defaults point to:

- API: `CYPRESS_API_BASE_URL=http://localhost:8080`
- Keycloak: `CYPRESS_KEYCLOAK_URL=https://keycloak.127.0.0.1.nip.io:8443`
- Realm: `CYPRESS_KEYCLOAK_REALM=expense-tracker-realm`
- Client ID: `CYPRESS_KEYCLOAK_CLIENT_ID=expense-tracker-ui`

Tests now reset and seed transactions, recurrent transactions, and invitations through API calls so each spec starts from a known state without relying on UI cleanup flows.

Run the suite with:

[source,shell]
npx nx e2e e2e --skip-nx-cache

=== Keycloak theme

The custom Keycloak login theme lives in link:projects/keycloak-theme/[projects/keycloak-theme].

For the most common local workflows:

- preview the lightweight no-`kcContext` shell in Vite: `npm run serve:keycloak-theme`
- preview mocked Keycloak pages in Storybook: `npm run storybook:keycloak-theme`
- build the frontend bundle only: `npm run build:keycloak-theme`
- build the Storybook preview: `npm run build-storybook:keycloak-theme`
- generate another mocked page story: `npm run add-story:keycloak-theme`
- build and package the deployable Keycloak theme: `npm run package:keycloak-theme`

See link:projects/keycloak-theme/README.md[projects/keycloak-theme/README.md] for the local dev notes, generated-file boundaries, and the files that matter most when editing the theme.
